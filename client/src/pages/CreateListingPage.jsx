...
export default function CreateListingPage() {
  ...
  const [form, setForm] = useState({
    name: '', category: '', description: '', photoUrl: '',
  });
  ...
  async function handleSubmit(e) {
    ...
    if (!location) { setError('Please set your location so neighbors can find this item'); return; }
    setError('');
    setLoading(true);
    try {
  ...
  return (
    <div className="container" style={{ maxWidth: 600 }}>
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: 14, marginBottom: 24 }}>
        Share something you rarely use with your neighbors.
      </p>
      ...
      <form onSubmit={handleSubmit} className="card">
        <div className="form-group">
          ...
          <input className="form-input" placeholder="e.g. Cordless drill" value={form.name} onChange={set('name')} required />
        </div>
        ...
        <div className="form-group">
          ...
          <input type="url" className="form-input" placeholder="https://…"
            value={form.photoUrl} onChange={set('photoUrl')} />
          <p className="form-hint">Paste a direct image URL (Imgur, Unsplash, etc.)</p>
        </div>
